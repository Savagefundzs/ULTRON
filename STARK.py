				...
				if pwx == '':
					...
					def manual(brute=None):
						...
						if ind =='':
							...
							print ('\n%s%s%s akun %s[Okenwa-OK] %stersimpan ke file %s> %sOK/%s.txt'%(U,til,O,H,O,M,H,waktu));jeda(0.2)
							print ('%s%s%s akun %s[%sOkenwa-CP%s]%s tersimpan ke file %s> %sCP/%s.txt'%(U,til,O,M,K,M,O,M,K,waktu));jeda(0.2)
